        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (data && data.password && data.savedPassword) {
                    // Handle both string and Buffer password types
                    let savedPassword = typeof data.savedPassword === 'string' ? data.savedPassword : Buffer.from(data.savedPassword, 'binary').toString();
                    isVerified = bcrypt.compareSync(data.password, savedPassword)
                }
