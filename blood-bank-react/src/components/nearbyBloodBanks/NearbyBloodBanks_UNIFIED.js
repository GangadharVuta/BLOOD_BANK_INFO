...
const getAvailabilityStatus = () => {
  const statuses = [
    ...
    { label: "Low Stock", type: "warning" },
    { label: "Critical", type: "danger" },
  ];
...
const NearbyBloodBanks = () => {
  ...
  return (
    <div className="nearby-banks-page">
      {/* Header */}
      <div className="nearby-banks__header">
        <h1>🩸 Find Nearby Blood Banks</h1>
        <p className="nearby-banks__subtitle">Search for blood banks and hospitals in your area</p>
      </div>

      <div className="container-fluid">
        {/* Search Section */}
        <div className="nearby-banks__search-wrapper">
          <form onSubmit={handleSearch} className="nearby-banks__search-form">
            <input
              type="text"
              value={locationInput}
              onChange={(e) => setLocationInput(e.target.value)}
              placeholder="Enter city or location..."
              className="nearby-banks__search-input"
              aria-label="Location search"
            />
            <select
              value={radius}
              onChange={(e) => setRadius(parseInt(e.target.value))}
              className="nearby-banks__search-select"
            ...
            <button type="submit" className="nearby-banks__search-btn" disabled={loading}>
              {loading ? "Searching..." : "Search"}
            </button>
          </form>

          {userLocation && (
            <div className="nearby-banks__location-info">
        ...
        {/* Error Message */}
        {error && <div className="nearby-banks__error">{error}</div>}

        {/* Loading Spinner */}
        {loading && (
          <div className="nearby-banks__loading">
            <div className="nearby-banks__spinner"></div>
              ...
              <div className="nearby-banks__map-section">
                ...
                >
                  ...
                  {bloodBanks.map((bank) => (
                    ...
                    >
                      <Popup>
                        <div className="nearby-banks__popup">
            ...
            <div className="nearby-banks__list-section">
              ...
              {bloodBanks.length > 0 ? (
                <>
                    ...
                    {displayedBanks.map((bank) => {
                      ...
                      return (
                        <DarkCard
                          ...
                          showCheckbox={false}
                          badge={{ text: availability.label, type: availability.type }}
                          title={bank.name}
                          ...
                          actions={[
                            ...
                            {
                              icon: isFavorited(bank.id) ? '❤️' : '🤍',
                              label: isFavorited(bank.id) ? 'Saved' : 'Save',
                              onClick: (e) => {
                                e.stopPropagation();
                                toggleFavorite(bank);
                              },
                  ...
                  {/* View More Button */}
                  {hasMoreResults && (
                    <div className="nearby-banks__view-more">
                      <button
